...
int fstoi( std::string pinp, int& pout );
...
//blocker if overflow of char in cin inputing on numbers
//flush is use when no error
void ftest_cin(  ){
      //L230 

      if( cin.fail(  ) ) {

            cin.clear(  ); //clear eror flags
            
            cin.ignore(1000, '\n');  //pag may error mattangal
                              //pati newline na over flow
                        //hindi kayang tangalin ng getchar() ang over flow
                  //pag galing sa cin
            return;
      }

      fflush(  );//pag wlang error may new line na over flow
}
...
void fadd_movie(  ) {
      ...
      cout << "\n\nsucessfully been saved the movie title " << blank_movie.movie_title << " and movie number " <<  blank_movie.movie_number << "\npress any...";
      getchar();
...
void flog_out( ){
      ...
      int seat_number, result;      
      cout << "\n\nlog out:\npress 0 to cancel, Enter seat number: ";
      cin >> ch;
      fflush( );

      result = fstoi(ch, seat_number);

      //menu is canceled
      if( result== -1 || seat_number == 0) {
            cout << "\n\nu press cancel or got error input of number...";
            getchar();
            return;
      }
      //get the real numbers
      ch = std::to_string( seat_number );

      //find in seat      
      seat_index = ffind_seat_number( seat_number, seat_status );
      //not found
      if( seat_status == -1 ){
...
void flog_in( ){
      ...
      cout << "\ndebug L21 result|" << result << "|, seat_number " <<  seat_number << "\n";

      //menu is cancelled
      if( result == -1 || seat_number == 0) {
            cout << "\n\nu press cancel or error input...";
            getchar();
            return;
      }
...
void fflush(  ){
      ...
      while( 1 ){
            ...
            //std::cout << "\ndebug L101 fflush  bef if (ch  ch |" <<  ch << "|\n"; 
